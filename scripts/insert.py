...
prompt = f"Generate {num_data_items} data items and create tables with the following schema: {schema_content} to create table and insert it to mysql database using python script using faker and use this connection: {cred_content} in valid python syntx and avoid long data "
